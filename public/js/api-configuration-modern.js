...
function createKeyValueRow(withCheckbox = false, key = '', value = '', enabled = true) {
    const row = document.createElement('div');
    row.className = 'kv-row';

    let html = `
        <input type="text" class="kv-input key-input" placeholder="Key" value="${escapeHtml(key)}">
        <input type="text" class="kv-input value-input" placeholder="Value" value="${escapeHtml(value)}">
    `;

    if (withCheckbox) {
        html += `<input type="checkbox" class="kv-checkbox" ${enabled ? 'checked' : ''}>`;
    } else {
        html += `<span></span>`;
    }
...
function renderFieldsList() {
    ...
    if (featureFields.length === 0) {
        container.innerHTML = '<div class="empty-state"><p>No fields found</p></div>';
        return;
    }
        ...
        html += `
            <div class="field-item ${isActive ? 'active' : ''}" onclick="selectField('${field.id}', '${field.type}')">
                <div class="field-header">
                    ...
                    <span class="field-status ${field.configured ? 'configured' : 'not-configured'}">
                        ${field.configured ? 'API' : 'NO API'}
                    </span>
                </div>
                <div class="field-type">${field.type === 'template' ? 'Template Field' : 'Extra Field'}</div>
            </div>
        `;
    });
...
function populateForm(config) {
    // Set basic fields
    document.getElementById('apiName').value = config.name || `${currentFieldId} API`;
    ...
    if (config.headers && config.headers.length > 0) {
        ...
        config.headers.forEach(header => {
            const row = createKeyValueRow(true, header.key, header.value, header.enabled !== false);
            headersList.appendChild(row);
        });
    }
    ...
    if (config.queryParams && config.queryParams.length > 0) {
        ...
        config.queryParams.forEach(param => {
            const row = createKeyValueRow(true, param.key, param.value, param.enabled !== false);
            paramsList.appendChild(row);
        });
    }
    ...
    if (config.bodyType) {
        ...
        const bodyTypeBtn = document.querySelector(`.body-type-btn[onclick*="${config.bodyType}"]`);
        if (bodyTypeBtn) {
...
async function loadUserConnections() {
    ...
    try {
        const response = await fetch(`/api/integrations/${integrationId}/user-connections?userId=${userId}`);
